...
namespace WebUI.Ashxs
{
    ...
    /// Valids 的摘要说明
    /// </summary>
    public class Valids : IHttpHandler,IRequiresSessionState
    {
        ...
        public void getghtoname(HttpContext context)
        {
            string lsgh = context.Request["lsgh"].ToString();
            string name="";
        ...
        public void getpwdvalid(HttpContext context)
        {
            string lsgh = context.Request["lsgh"].ToString();
            string lspwd = context.Request["lspwd"].ToString();
            Boolean lbnisv;
            lbnisv = Convert.ToBoolean(Convert.ToInt32(new Dbopr().getstr("SELECT COUNT(*) FROM RS_USERS WHERE LS_GH = :LS_GH AND LS_PWD=:LS_PWD", new OracleParameter[] { new OracleParameter(":LS_GH", lsgh), new OracleParameter(":LS_PWD", lspwd) })));
            context.Response.Write(lbnisv);
        }
        ...
        public void logandrec(HttpContext context)
        {
            ...
            using (OracleDataReader dr = new Dbopr().getdr("SELECT * FROM RS_USERS WHERE LS_GH = :LS_GH AND LS_PWD=:LS_PWD", new OracleParameter[] { new OracleParameter(":LS_GH", lsgh), new OracleParameter(":LS_PWD", lspwd) }))
            {
                if (dr.Read())
                {
                    ...
                    user.LS_GH = dr["LS_GH"].ToString();
                    user.LS_XM = dr["LS_XM"].ToString();
                    user.LS_BZ = dr["LS_BZ"].ToString();
                    user.LS_QX = dr["LS_QX"].ToString();
                    object obj = dr["LB_ZP"];
                    if (obj != System.DBNull.Value) { user.LB_ZP = (byte[])obj; }
                    context.Session["user"] = user;
                    result = "success";
        ...
        {
            DataTable dt = new Dbopr().getds("SELECT * FROM PB_MMENU ORDER BY PNODE,NODE").Tables[0];
            string json = GetTreeJsonByTable(dt, "NODE", "TEXT", "TAG", "PNODE", "0");
            context.Response.Write(json);
        }
        public void getpartmtree(HttpContext context)
        {
            DataTable dt = new Dbopr().getds("SELECT * FROM RS_BMTV ORDER BY PARENTNODE,NODE").Tables[0];
            string json = GetTreeJsonByTable(dt, "NODE", "DISPLAYTEXT", "TAGDATA", "PARENTNODE", "0");
            context.Response.Write(json);
        }
...
;
        ...
        {
            int total = 0;
            string sqlstr = "SELECT * FROM PB_MMENU ORDER BY PNODE,NODE";
            DataTable dt = new Dbopr().getds(sqlstr).Tables[0];
